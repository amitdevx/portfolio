...
date: "2026-02-23"
updated: "2026-02-23"
description: "Amit Divekar explains: How I built an intelligent database documentation platform using Next.js, FastAPI, and DeepSeek-V3 that saves engineering teams hours of manual work while improving code quality and onboarding."
image: "/blogs/schemasense/schemasense-dashboard.png"
...
![SchemaSense Dashboard](/blogs/schemasense/schemasense-dashboard.png)
...
graph TB
    ...
    subgraph Backend[" BACKEND (FastAPI + Python 3.11)"]
        ...
        subgraph Cache[" SQLite Cache Layer"]
            CacheDB["AI Response Caching<br/>• SHA256-based keys<br/>• 95% cost reduction<br/>• Sub-50ms lookups"]
        end
...
async def get_cached_explanation(table_name: str, connection_hash: str, 
    ...
    logger.info(f" Cache MISS for {table_name}")
    return None
...
graph LR
    ...
    subgraph CacheKey[" Cache Key Generation"]
        Hash["Generate Key"]
    ...
    Check -->|YES| CacheHit[" CACHE HIT<br/>Return in 45ms"]
    Check -->|NO| CacheMiss[" CACHE MISS"]
...
![SchemaSense Chat Interface](/blogs/schemasense/schemasense-chat-interface.png)
...
graph TD
    ...
    subgraph Layer1[" LAYER 1: Pattern Detection"]
    ...
    subgraph Layer2[" LAYER 2: Behavioral Analysis"]
    ...
    subgraph Layer3[" LAYER 3: Message Separation"]
    ...
    PatternCheck -->|2+ patterns| Blocked[ BLOCKED<br/>HTTP 400]
    PatternCheck -->|1 pattern| Warning[ WARNING<br/>Log & Continue]
...
async def connect_database(request: ConnectionRequest):
    ...
    logger.info(f" Connection pool created for {connection_id}")
